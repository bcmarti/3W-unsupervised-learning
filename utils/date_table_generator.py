...
# well_number -> {"start": datetime, "end": datetime}
well_ranges = defaultdict(lambda: {"start": None, "end": None})

def update_range(well_num, date_min, date_max):
    entry = well_ranges[well_num]
...
for i, (root, filename) in enumerate(all_files, 1):
    ...
    try:
        # Read only the index (timestamp) — much faster than loading all columns
        ...
        update_range(well_num, index.min(), index.max())
        files_processed += 1
...
# Build output CSV
rows = []
for well_num in sorted(well_ranges):
    entry = well_ranges[well_num]
    rows.append({
...
output_df = pd.DataFrame(rows, columns=["well", "start_date", "end_date"])
